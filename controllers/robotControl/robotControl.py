...
class RobotController:
    ...
    def setup(self):
        ...
        # Apply the boundaries to the SLAM map
    ...
    def step(self):
        ...
        # Update SLAM and get position from SLAM
        self.slam_manager.update()
        current_state = self.slam_manager.get_position()
        
        # Display the SLAM map every 5 seconds
        ...
        if self.state.get_position().get('obstacle_detected', False):
            ...
            print(f"Obstacle detected at distance: {self.state.get_position()['obstacle_distance']:.2f}m")
