...
MOTOR_PINS = {
    'left_motor': {
        'pwm_pin1': 12,      # PWM pin for left motor speed control
        'dir1_pin': 27,     # Direction pin 1 for left motor
    ...
    'right_motor': {
        'pwm_pin1': 13,      # PWM pin for right motor speed control
        'dir1_pin': 20,     # Direction pin 1 for right motor
...
SENSOR_PINS = {
    'laser_range': {
        'rx_pin': 15,       # Serial RX pin for laser range finder (or use None for I2C/hardware UART)
        'tx_pin': 14,       # Serial TX pin for laser range finder (or use None for I2C/hardware UART)
        'uart_port': '/dev/ttyAMA0',  # Hardware UART port (alternative to GPIO pins)
        'i2c_address': None   # I2C address if using I2C interface
    },
    'compass': {
...
ROBOT_PARAMS = {
    ...
    'encoder_ticks_per_rev': 400,  # Encoder ticks per wheel revolution
